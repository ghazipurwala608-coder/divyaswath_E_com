...
test('admin creates and assigns a partner; only an active assigned driver can update the order', async () => {
  ...
  assert.equal((await request(`/orders/${orderId}/status`, admin, 'PUT', { orderStatus: 'Shipped' })).status, 400)
  assert.equal((await request('/auth/login', null, 'POST', { email: 'customer@delivery.example', password: 'DeliveryTest!123', portal: 'delivery' })).status, 403)
  const portalLogin = await request('/auth/login', null, 'POST', { email: 'partner@delivery.example', password: 'DeliveryTest!123', portal: 'delivery' })
  assert.equal(portalLogin.status, 200)
  assert.equal(portalLogin.data.user.deliveryArea, 'Noida 201301')
  ...
  assert.equal((await request('/auth/login', null, 'POST', { email: 'partner@delivery.example', password: 'DeliveryTest!123', portal: 'delivery' })).status, 403)
...
test('admin edits and deletes partners while protecting access and active deliveries', async () => {
  const partner = await User.create({ name: 'Editable partner', email: 'editable@delivery.example', phone: '9876543210', password: 'DeliveryTest!123', isDriver: true })
  const token = generateToken(partner._id)
  const path = `/delivery/team/${partner.id}`
  assert.equal((await request(path, customer, 'PUT', { name: 'No access' })).status, 403)
  assert.equal((await request(path, customer, 'DELETE')).status, 403)
  assert.equal((await request(path, admin, 'PUT', { phone: '123' })).status, 400)
  assert.equal((await request(path, admin, 'PUT', { email: 'admin@delivery.example' })).status, 409)
  const updated = await request(path, admin, 'PUT', { name: 'Updated partner', email: 'updated@delivery.example', phone: '9876501234', deliveryArea: 'Delhi 110020', password: 'NewDelivery!123', isAdmin: true })
  assert.equal(updated.status, 200)
  assert.equal(updated.data.driver.name, 'Updated partner')
  assert.equal(updated.data.driver.deliveryArea, 'Delhi 110020')
  assert.equal(updated.data.driver.password, undefined)
  assert.equal((await User.findById(partner.id)).isAdmin, false)
  assert.equal((await request('/auth/login', null, 'POST', { email: 'updated@delivery.example', password: 'NewDelivery!123' })).status, 200)
  const assigned = await Order.create({ user: partner._id, deliveryPerson: partner._id, shippingAddress: address, items: [{ product: 'test-product', slug: 'test-product', name: 'Test product', price: 500, quantity: 1 }], itemsPrice: 500, shippingPrice: 0, totalPrice: 500, orderStatus: 'Packed' })
  assert.equal((await request(path, admin, 'DELETE')).status, 409)
  assigned.orderStatus = 'Delivered'
  await assigned.save()
  assert.equal((await request(path, admin, 'DELETE')).status, 200)
  assert.equal(await User.findById(partner.id), null)
  assert.ok(await Order.findById(assigned.id))
  assert.equal((await request('/delivery/orders', token)).status, 401)
  assert.equal((await request(path, admin, 'DELETE')).status, 404)
})
