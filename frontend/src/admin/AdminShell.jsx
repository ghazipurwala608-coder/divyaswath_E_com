...
export default function AdminShell() {
  ...
  return <div className="admin-app">{mobile && <button className="admin-nav-backdrop" aria-label="Close menu" onClick={() => setMobile(false)} />}<aside className={`admin-sidebar ${mobile ? 'open' : ''}`}><Link to="/admin" className="admin-brand"><span className="admin-brand-logo"><img src="https://res.cloudinary.com/djfluwhwu/image/upload/v1789450628/divyaswasth/migrated/1282459c18c7948b-logo.jpg" alt="Divya Swasth logo" /></span><div>DIVYA <b>SWASTH</b><small>STORE ADMINISTRATION</small></div></Link><button className="admin-mobile-close" aria-label="Close navigation" onClick={() => setMobile(false)}><X /></button><p className="admin-nav-label">WORKSPACE</p><nav aria-label="Administration">{tabs.map(([key, label, Icon]) => <button key={key} className={tab[0] === key ? 'active' : ''} aria-current={tab[0] === key ? 'page' : undefined} onClick={() => select(key)}><Icon size={18} /><span>{label}</span>{tab[0] === key && <i />}</button>)}</nav><div className="admin-sidebar-bottom"><div className="admin-sidebar-note"><Leaf size={22} /><strong>Rooted in care.</strong><p>Growing your store, one thoughtful detail at a time.</p></div><Link to="/" target="_blank">View storefront <ArrowUpRight size={16} /></Link><button onClick={() => { logout(); navigate('/login') }}><LogOut size={16} /> Sign out</button></div></aside>
    <div className="admin-workspace"><header className="admin-topbar"><div><button className="admin-mobile-toggle" aria-label="Open navigation" onClick={() => setMobile(true)}><Menu /></button><span>Workspace <b>/ {tab[1]}</b></span></div><div className="admin-user"><span className="admin-live-dot" /><small>Store management</small><AdminUserMenu user={user} onLogout={() => { logout(); navigate('/login') }} /></div></header><main className="admin-main"><div className="admin-page-heading"><div><span className="admin-eyebrow">DIVYA SWASTH / ADMIN</span><h1>{tab[1]}</h1><p>{tab[3]}</p></div><span className="admin-date">{new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}</span></div>
